...
import { CheckCircleIcon, ExclamationCircleIcon, TrashIcon, XCircleIcon } from '@heroicons/react/solid'

export default function Update() {
  const { currentUser, sendPasswordResetEmail, deleteUser} = useAuth()
  ...
  async function removeUser(event) {
      ...
      try {
          await database.users.doc(currentUser.email).collection('screenings').get().then((querySnapshot) => {
              querySnapshot.forEach((doc) => {
                  database.users.doc(currentUser.email).collection('screenings').doc(doc.id).delete()
              })
          }) 
          await database.users.doc(currentUser.email).delete().then(() => {
              deleteUser()
          })
  ...
  async function resetPassword(event) {
    ...
    try {
      ...
      setPmess('Password reset email sent to ' + currentUser.email)
    } catch {
    ...
    <div className="bg-white">
      ...
      <div className="py-10">
        ...
        <header className="mt-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-4">
            <h1 className="text-3xl font-bold leading-tight text-gray-900">Manage Account</h1>
          </div>
        </header>
        <main>
          <div className="z-50 max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div className="space-y-6 lg:px-0 lg:col-span-9">
                <div className="shadow sm:rounded-md overflow-hidden shadow border border-gray-200">
                  <div className="bg-white py-6 px-4 space-y-6 sm:p-6">
                    ...
                    {pmess && <div className="mt-5 rounded-md bg-green-50 p-4">
                        <div className="flex">
                            <div className="flex-shrink-0">
                                <CheckCircleIcon className="h-5 w-5 text-green-400" aria-hidden="true" />
                            </div>
                            <div className="ml-3">
                                <h3 className="text-sm text-green-800">{pmess}</h3>
                            </div>
                        </div>
                    </div>}

                    <div onClick={removeUser} className="cursor-pointer mt-5 rounded-md bg-red-50 p-4">
                        <div className="flex">
                            <div className="flex">
                                <TrashIcon className="h-5 w-5 text-red-400" aria-hidden="true" />
                            </div>
                            <div className="ml-3">
                                <h3 className="text-sm text-red-800">Delete this account - Not recommended (You will lose all previous screening data)</h3>
                            </div>
                        </div>
                    </div>
                </div>
